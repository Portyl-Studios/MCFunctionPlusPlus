...
import { tokenizeCommandWithRanges } from "./parse-utils"
...
/**
 * Live linting diagnostics for real-time validation as the user types.
 * These checks don't require context index and include:
 * - Unknown command validation
 * - Macro variable usage warnings
 */
export const mcfunctionLiveLintingSource = (view: EditorView): Diagnostic[] => {
  ...
  for (let lineNumber = 1; lineNumber <= doc.lines; lineNumber += 1) {
    ...
    if (!isContinuationLine && commandMatch && commandMatch[1]) {
